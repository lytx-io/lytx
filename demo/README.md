...
- `src/Document.tsx` and `src/client.tsx` are optional with `lytxConsumerVitePlugin()`; defaults come from `lytx`, and custom document wrappers should be passed via `createLytxApp({ routes: { document } })`.
