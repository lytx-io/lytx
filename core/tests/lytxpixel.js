...
var lytxpixel = (() => {
  ...
  var lytxpixel_exports = {};
  __export(lytxpixel_exports, {
    decodedPixel: () => decodedPixel,
    parseData: () => parseData
  });
  ...
  function parseData(data = null, config, trackCustomEvents, track_web_events, platformName) {
    ...
    if (window.lytxDataLayer.length < 2) {
      console.log(`Lytx script is working \u{1F525}\u{1F525}\u{1F525}${debug ? "\u{1F41B}\u{1F41B}\u{1F41B} debug enabled" : ""}`);
    }
    ...
    function googleConversion(GoogleCommand, id, type, value) {
      ...
      if (GoogleCommand == "config") {
        ...
        if (type == "conversion") {
          try {
            const gtag_report_conversion = (url = void 0) => {
                      ...
                      if (parseConfig.path) {
                        ...
                        if (path.includes(`${parseConfig.path}`) || parseConfig.path == "*") {
                          if (parseConfig.querySelectorAll == "all") {
                            ...
                            if (debug) {
                              console.log(`\u{1F41B}\u{1F41B}\u{1F41B} ${ev.event_name} has the following elements tracked`, allElems);
                            }
                          } else {
  ...
  return __toCommonJS(lytxpixel_exports);
})();
