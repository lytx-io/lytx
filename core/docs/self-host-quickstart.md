...
- `AI_API_KEY`, `AI_ACCOUNT_ID`, `AI_PROVIDER`, `AI_MODEL`, `AI_BASE_URL`, `AI_DAILY_TOKEN_LIMIT` (AI features)
...
- auth controls: `auth.signupMode`, `auth.emailPasswordEnabled`, `auth.requireEmailVerification`, `auth.socialProviders.google`, `auth.socialProviders.github`
